module.exports = function (app, connection) {
    app.route('/api/pub/blogs')
        ...
        .get(function (req, res, next) {
            connection.query(
                'SELECT * FROM whatcha.`blogs-list`',
                (error, result, fields) => {
        ...
        .post(function (req, res, nex) {
            ...
            connection.query(
                'INSERT INTO whatcha.`blogs-list` (`uid`, `title`, `content`) values(?,?, ?)',
                [uid, title, content],
    ...
    app.route('/api/pub/blogs/:blogId')
        ...
        .get(function (req, res, next) {
            ...
            connection.query(
                `SELECT * FROM whatcha.\`blogs-list\` WHERE \`blogId\` = ${blogId}`,
                function (error, result, fields) {
                    res.header('Access-Control-Allow-Origin', '*');
                    res.header(
                        'Access-Control-Allow-Headers',
        ...
        .delete(function (req, res, next) {
            const blogId = req.params.blogId;
            connection.query(
                `DELETE FROM whatcha.\`blogs-list\` WHERE \`blog_id\` = ${blogId}`,
                function (error, result, fields) {
                    res.header('Access-Control-Allow-Origin', '*');
                    res.header(
                        'Access-Control-Allow-Headers',
